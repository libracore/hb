...
def find_closest_hotels(object_name):
    query = """
        SELECT 
            ...
            `main_hotel`, 
            `remarks`,
            (SQRT(POW(2.65 * ABS(`gps_latitude` - {lat}), 2) + POW(ABS(`gps_longitude` - {lon}), 2)) / POW(5, `main_hotel`)) AS `prox`,        /* this is an approximation function by gps coordinates and a numeric factor in arbitrary units */
            `gps_latitude`, 
            `gps_longitude`
        FROM `tabSupplier`
        WHERE 
...
def find_closest_parkings(object_name):
    query = """
        SELECT 
            ...
            `gps_latitude`,
            `gps_longitude`,
            (SQRT(POW(2.65 * ABS(`gps_latitude` - {lat}), 2) + POW(ABS(`gps_longitude` - {lon}), 2))) AS `prox`        /* this is an approximation function by gps coordinates and a numeric factor in arbitrary units */
        FROM `tabParking`
        WHERE 
...
def find_closest_troughs(object_name):
    ...
    query = """
        SELECT 
            ...
            (SQRT(POW(2.65 * ABS(`tabSupplier Activity`.`gps_lat` - {lat}), 2) + POW(ABS(`tabSupplier Activity`.`gps_long` - {lon}), 2)) / POW(5, `tabSupplier`.`main_hotel`)) AS `prox`,        /* this is an approximation function by gps coordinates and a numeric factor in arbitrary units */
            ...
            `tabSupplier Activity`.`default_trough_size`,
            `tabSupplier Activity`.`place_trough_rate`,
            `tabSupplier Activity`.`trough_pickup_rate`,
            `tabSupplier Activity`.`disposal_rate`,
            `tabSupplier Activity`.`transport_rate_mud`,
